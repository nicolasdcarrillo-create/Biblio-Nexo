...
`);this._descargar(`﻿`+i,`reporte-biblionexo-${t.desde}-a-${t.hasta}.csv`,`text/csv;charset=utf-8;`),this.showToast(`Reporte exportado.`,`success`)}},G=[{valor:`1`,etiqueta:`Normal`},{valor:`1.15`,etiqueta:`Grande`},{valor:`1.3`,etiqueta:`Muy grande`}];function Je(){let e=`1`;try{e=localStorage.getItem(`biblionexo-escala-fuente`)||`1`}catch{}let t=G.findIndex(t=>t.valor===e);return t===-1?0:t}function Ye(e){let t=G[e];document.documentElement.style.setProperty(`--escala-fuente`,t.valor);try{localStorage.setItem(Be,t.valor)}catch{}}var Xe={async renderProfile(){let e=this._container();if(!e)return;let t=null;try{t=await V.miPerfil()}catch(e){console.warn(`Perfil no disponible:`,e.message)}if(this.currentView!==`profile`)return;if(!t){e.innerHTML=this._avisoMigracion(`008`,`008_perfiles_y_permisos_librero.sql`);return}this._perfil=t;let n=l.ROLE_LABELS[t.rol]||l.ROLE_LABELS.librero,i=(t.nombre||t.email||`?`).trim().charAt(0).toUpperCase(),a=e=>e?new Date(e).toLocaleString(`es-CL`,{day:`numeric`,month:`long`,year:`numeric`,hour:`2-digit`,minute:`2-digit`}):`Nunca`,o=(e,t,n,i=``,a=``)=>r`
    ...
    `,e.querySelectorAll(`.admin-tab-btn`).forEach(e=>{e.addEventListener(`click`,()=>{this.adminTab=e.dataset.adminTab,this.renderAdmin()})});let i=document.getElementById(`admin-panel`),a={inventario:()=>this._adminInventario(i),reservas:()=>this._adminReservas(i),bloqueados:()=>this._adminBloqueados(i),personal:()=>this._adminPersonal(i),enlaces:()=>this._adminEnlacesEscaneo(i),eliminados:()=>this._adminEliminados(i),auditoria:()=>this._adminAuditoria(i),cumplimiento:()=>this._adminCumplimiento(i),diagnostico:()=>this._adminDiagnostico(i)};try{await(a[t]||a.inventario)()}catch(e){i.innerHTML=r`<div class="catalog-card bg-patrimonio-card dark:bg-stone-900 rounded-2xl border border-stone-300 dark:border-stone-600 p-6">
      ...
      </div>`},async _adminCumplimiento(e){let[t,n,i,a]=await Promise.all([K.verificarRls(),V.obtenerParametros(),K.verificarCirculacion(),K.obtenerRespaldos(5)]);if(t===null||n===null){e.innerHTML=this._avisoMigracion(`007`,`007_correcciones_y_cumplimiento_legal.sql`);return}let o=t.filter(e=>e.diagnostico!==`Correcto`),c=(i||[]).filter(e=>!e.es_definer),l=a[0]||null;e.innerHTML=r`
      <div class="space-y-4">
      ...
      <div class="bg-white dark:bg-stone-800 rounded-2xl p-4 shadow-sm border border-stone-200 dark:border-stone-700 flex flex-col md:flex-row gap-4 items-start md:items-center animate-fade-up" style="animation-delay: ${t*.05}s">
        ...
        <div class="flex flex-col md:items-end gap-3 shrink-0">
          ...
          <div class="flex flex-wrap gap-2 justify-end">
            ${e.stock>0?r`<button class="loan-book-btn bg-patrimonio-madera text-white px-5 py-2.5 rounded-full text-sm font-semibold shadow-md hover:bg-[#5E3214] transition-all hover:scale-105 active:scale-95" data-id="${e.id}"><i aria-hidden="true" class="fas fa-hand-holding-hand mr-1"></i> Prestar</button>`:r`<button class="reserve-book-btn btn-secundario px-4 py-2 rounded-xl text-xs font-bold text-patrimonio-lago border border-stone-300 dark:border-stone-600 hover:bg-stone-50 dark:hover:bg-stone-700 transition" data-id="${e.id}"><i aria-hidden="true" class="fas fa-bookmark mr-1"></i> Reservar</button>`}
    ...
    `.toString(),document.body.appendChild(t);let n=this._prepararModal(t);t.querySelector(`[data-action="close"]`).addEventListener(`click`,n),t.addEventListener(`click`,e=>{e.target===t&&n()}),t.querySelectorAll(`[data-notify-id]`).forEach(t=>{t.addEventListener(`click`,()=>{let n=e.find(e=>String(e.id)===String(t.dataset.notifyId));n&&this.showNotifyModal(n)})})},async _seleccionarLectorModal(e){return new Promise(n=>{let r=document.createElement(`div`);r.className=`fixed inset-0 bg-patrimonio-lago/50 backdrop-blur-sm z-[10000] flex items-center justify-center p-4`,r.innerHTML=`
        <div class="bg-patrimonio-card dark:bg-stone-900 border border-stone-300 dark:border-stone-600 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
          <h3 class="font-serif text-lg font-bold text-stone-900 dark:text-stone-100">${t(e)}</h3>
      ...
      `,document.body.appendChild(r);let i=r.querySelector(`#lector-search-input`),a=r.querySelector(`#lector-search-results`),o=r.querySelector(`#lector-search-confirm`),s,c=null,l=()=>{r.remove(),n(null)},u=(e,n)=>{c=e,i.value=e,a.innerHTML=`
          <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-emerald-800 text-sm">
            <i aria-hidden="true" class="fas fa-check-circle mr-1.5"></i> ${t(n)}
          </div>`,o.disabled=!1,o.focus()};r.querySelector(`#lector-search-cancel`).addEventListener(`click`,l),r.addEventListener(`click`,e=>{e.target===r&&l()}),o.addEventListener(`click`,()=>{let e=c||i.value.trim();r.remove(),n(e)}),i.addEventListener(`input`,()=>{let e=i.value;if(/^[0-9kK\-\.]+$/.test(e)){let t=e.replace(/[.\-\s]/g,``).toUpperCase();t.length>1&&(i.value=`${t.slice(0,-1)}-${t.slice(-1)}`)}o.disabled=!i.value.trim(),c=null,clearTimeout(s);let n=i.value.trim();if(n.length<2){a.innerHTML=``;return}s=setTimeout(async()=>{a.innerHTML=`<p class="text-xs text-stone-500 dark:text-stone-400 p-2"><i aria-hidden="true" class="fas fa-spinner fa-spin mr-1"></i> Buscando...</p>`;try{let e=await Y.obtenerLectores(n,0,5);e.lectores.length===0?a.innerHTML=`<p class="text-xs text-stone-500 dark:text-stone-400 p-2">Ningún lector coincide. Escriba el RUT completo para registrarlo como nuevo.</p>`:(a.innerHTML=e.lectores.map(e=>`
                <button type="button" data-rut="${e.rut}" data-nombre="${t(e.nombre)}" class="w-full text-left px-3 py-2 rounded-lg border border-transparent hover:bg-stone-50 dark:bg-stone-800/50 hover:border-stone-200 dark:border-stone-700 focus:bg-stone-50 dark:bg-stone-800/50 focus:border-stone-200 dark:border-stone-700 focus:outline-none transition-colors">
                  <p class="text-sm font-medium text-stone-800 dark:text-stone-200">${t(e.nombre)}</p>
                  <p class="text-[11px] font-mono text-stone-500 dark:text-stone-400">${e.rut}</p>
                </button>
              `).join(``),a.querySelectorAll(`button`).forEach(e=>{e.addEventListener(`click`,()=>u(e.dataset.rut,e.dataset.nombre))}))}catch{a.innerHTML=`<p class="text-xs text-rose-500 p-2">Error al buscar.</p>`}},350)}),i.addEventListener(`keydown`,e=>{e.key===`Enter`&&!o.disabled&&o.click()}),setTimeout(()=>i.focus(),100)})},async flujoPrestamo(e,t){let n=await this._seleccionarLectorModal(`Prestar libro`);if(!n)return;if(!this.isValidRut(n)){this.showToast(`El RUT no es válido. Revisa el dígito verificador.`,`error`);return}let r;try{r=await Y.estadoLector(this.formatRut(n))}catch(e){this.showToast(e.message||`No se pudo consultar el lector.`,`error`);return}this.showConfirmarPrestamoModal(e,this.formatRut(n),r,t)},showConfirmarPrestamoModal(e,t,n,i){let a=document.createElement(`div`);a.className=`fixed inset-0 bg-patrimonio-lago/50 backdrop-blur-sm z-[10000] flex items-center justify-center p-4`;let o,s;n.existe?n.puede_prestar?(o=r`
    ...
    `;let n=async e=>{let n=document.getElementById(`scan-result`);if(n){this._ultimoCodigoEscaneado=e,n.innerHTML=`<div class="flex items-center gap-2 text-sm text-stone-500 dark:text-stone-400"><i aria-hidden="true" class="fas fa-spinner fa-spin text-patrimonio-lago"></i> Consultando…</div>`;try{let t=await q.consultarLibro(e);if(!t){await this._formularioAltaRapida(n,e);return}let r=document.getElementById(`fast-return-toggle`);if(r&&r.checked){let e=t.prestamos.find(e=>!e.fecha_devolucion_real);if(e)try{await H.devolverPrestamo(e.id),this.showToast(`Devolución rápida exitosa.`,`success`),n.innerHTML=`<div class="m-auto text-center py-12 text-stone-500 dark:text-stone-400">
      ...
      <div class="bibliomovil-card bg-patrimonio-card dark:bg-stone-900 rounded-2xl shadow-sm border border-stone-300 dark:border-stone-600 overflow-x-auto">
        ...
        <div id="bibliomovil-tbody" class="flex flex-col gap-4 p-4">${this._renderBookRows(this._filtrarLibros(o))}</div>
        <div id="bibliomovil-pagination">${s(this._paginacionHtml(this.bookPage,c,t,`bibliomovil-page-btn`))}</div>
      </div>
    `,this._booksCache=o,document.getElementById(`add-book-form`).addEventListener(`submit`,async e=>{if(e.preventDefault(),!this.validateBookForm(!1))return;let t=document.getElementById(`add-book-submit-btn`),n=t.innerHTML;t.disabled=!0,t.innerHTML=`<i aria-hidden="true" class="fas fa-spinner fa-spin mr-1"></i> Guardando...`;try{let e=await q.agregarLibro({isbn:document.getElementById(`new-book-isbn`).value.trim(),titulo:document.getElementById(`new-book-title`).value.trim(),autor:document.getElementById(`new-book-author`).value.trim(),genero:document.getElementById(`new-book-genre`).value.trim(),ubicacion:document.getElementById(`new-book-location`).value.trim(),stock:Number(document.getElementById(`new-book-qty`).value||1)});this.showToast(e?.encolado?e.mensaje:`Libro agregado.`,e?.encolado?`info`:`success`),this.renderBibliomovil()}catch(e){this.showToast(e.message||`No se pudo agregar el libro.`,`error`)}finally{t&&(t.disabled=!1,t.innerHTML=n)}}),this._bindCatalogRowEvents(e),this._bindPaginacion(e,`.bibliomovil-page-btn`,e=>{this.bookPage=e,this.renderBibliomovil()}),e.querySelectorAll(`.bibliomovil-filter-btn`).forEach(e=>{e.addEventListener(`click`,()=>{this.bibliomovilFilter=e.dataset.filter,this.bookPage=0,this.renderBibliomovil()})});let l=document.getElementById(`bibliomovil-search-input`);l.addEventListener(`input`,()=>{clearTimeout(this._bibliomovilSearchTimer),this._bibliomovilSearchTimer=setTimeout(async()=>{this.bibliomovilSearch=l.value.trim(),this.bookPage=0;let n=m.from(`libros`).select(`*`,{count:`exact`}).ilike(`ubicacion`,`%bibliom%`).order(`titulo`,{ascending:!0}).range(0,t-1);this.bibliomovilSearch&&(n=n.or(`titulo.ilike.%${this.bibliomovilSearch}%,autor.ilike.%${this.bibliomovilSearch}%`));let{data:r,count:i}=await n,a=r||[],o=i||0,s=document.getElementById(`bibliomovil-tbody`);if(this.currentView!==`bibliomovil`||!s)return;this._booksCache=a,s.innerHTML=this._renderBookRows(this._filtrarLibros(a)).toString();let c=document.getElementById(`bibliomovil-pagination`);c&&(c.innerHTML=this._paginacionHtml(0,o,t,`bibliomovil-page-btn`),this._bindPaginacion(e,`.bibliomovil-page-btn`,e=>{this.bookPage=e,this.renderBibliomovil()})),this._bindCatalogRowEvents(e)},350)})},_filtrarLibros(e){let t=this.bibliomovilFilter||`todos`;return t===`disponibles`?e.filter(e=>e.stock>0):t===`prestados`?e.filter(e=>e.stock===0):e},_renderBookRows(e){return e.length?r`${e.map((e,t)=>r`
      <div class="bg-white dark:bg-stone-800 rounded-2xl p-4 shadow-sm border border-stone-200 dark:border-stone-700 flex flex-col md:flex-row gap-4 items-start md:items-center animate-fade-up" style="animation-delay: ${t*.05}s">
    ...
    `)}`:r`<div class="px-4 py-6 text-center text-stone-500 dark:text-stone-400">Sin libros que coincidan con la búsqueda.</div>`},_bindCatalogRowEvents(e){e.querySelectorAll(`.delete-book-btn`).forEach(e=>{e.addEventListener(`click`,async()=>{if(await this.showConfirm(`¿Eliminar este libro? Esta acción no se puede deshacer.`,{title:`Eliminar libro`,confirmText:`Eliminar`}))try{await q.eliminarLibro(e.dataset.id),this.showToast(`Libro eliminado.`,`success`),this.renderBibliomovil()}catch(e){this.showToast(e.message||`No se pudo eliminar.`,`error`)}})}),e.querySelectorAll(`.loan-book-btn`).forEach(e=>{e.addEventListener(`click`,()=>this.promptCreateLoan(e.dataset.id))}),e.querySelectorAll(`.reserve-book-btn`).forEach(e=>{e.addEventListener(`click`,()=>this.promptCreateReserva(e.dataset.id))}),e.querySelectorAll(`.edit-book-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=(this._booksCache||[]).find(t=>String(t.id)===String(e.dataset.id));t&&this.showEditBookModal(t)})})},showEditBookModal(e){let t=document.createElement(`div`);t.className=`fixed inset-0 bg-patrimonio-lago/50 backdrop-blur-sm z-[10000] flex items-center justify-center p-4`;let n=(e,t,n,i=``)=>r`
      <div>
        <label for="${e}" class="text-[11px] font-black uppercase tracking-wide text-stone-600 dark:text-stone-300 mb-1 block">${t}</label>
        <input id="${e}" value="${n??``}" ${s(i)}
          class="w-full px-3 py-2 border border-stone-300 dark:border-stone-600 rounded-md bg-white dark:bg-stone-800 text-sm focus:outline-none focus:border-patrimonio-lago focus:ring-1 focus:ring-patrimonio-lago" />
      ...
      </div>`.toString(),document.body.appendChild(t);let i=this._prepararModal(t);t.querySelector(`[data-action="cancel"]`).addEventListener(`click`,i),t.addEventListener(`click`,e=>{e.target===t&&i()}),t.querySelector(`[data-action="save"]`).addEventListener(`click`,async t=>{if(!this.validateBookForm(!0))return;let n=t.currentTarget;n.disabled=!0;try{let t=document.getElementById(`edit-book-plazo`).value.trim();await q.actualizarLibro(e.id,{titulo:document.getElementById(`edit-book-title`).value.trim(),autor:document.getElementById(`edit-book-author`).value.trim(),isbn:document.getElementById(`edit-book-isbn`).value.trim(),genero:document.getElementById(`edit-book-genre`).value.trim(),ubicacion:document.getElementById(`edit-book-location`).value.trim(),portada_url:document.getElementById(`edit-book-cover`).value.trim(),diasPrestamoOverride:t===``?null:Number(t)});let n=Number(document.getElementById(`edit-book-qty`).value||0);n!==(e.copias_totales??e.stock)&&await q.ajustarCopias(e.id,n),i(),this.showToast(`Libro actualizado.`,`success`),this.renderBibliomovil()}catch(e){this.showToast(e.message||`No se pudo guardar.`,`error`),n.disabled=!1}})},async promptCreateLoan(e){await this.flujoPrestamo(e,()=>{this.currentView===`catalog`&&this.renderBibliomovil()})},async promptCreateReserva(e){await this.flujoReserva(e,()=>{this.currentView===`catalog`&&this.renderBibliomovil()})}});var $=new He,ot=`No se pudo iniciar la aplicación a tiempo. Recargue la página; si el problema sigue, cierre esta pestaña y ábrala de nuevo.`;function st(){try{Object.keys(localStorage).filter(e=>e.startsWith(`sb-`)&&e.endsWith(`-auth-token`)).forEach(e=>localStorage.removeItem(e))}catch{}}function ct(){return(window.location.hash||``).includes(`type=recovery`)}function lt(){return(window.location.hash||``).includes(`type=invite`)}var ut=3e5,dt=!1;function ft(){dt||(dt=!0,Re.iniciar(),E.sincronizarTodo(),R.reintentarPendientes(),setInterval(()=>{E.sincronizarTodo(),R.reintentarPendientes()},ut),window.addEventListener(`online`,()=>E.sincronizarTodo()))}document.addEventListener(`DOMContentLoaded`,async()=>{D.iniciar(()=>`arranque`);try{if(!m)throw Error(`No se pudo inicializar la conexión con Supabase (CDN no disponible).`);if(ct()){$.renderNuevaPassword(),window.__appBooted=!0;return}if(lt()){$.renderCompletarInvitacion(),window.__appBooted=!0;return}let e=null,t=async t=>{if(e){await e;return}e=$.renderShell(t),await e,ft()};ae(async(n,r)=>{if(n===`PASSWORD_RECOVERY`){$.renderNuevaPassword();return}n===`SIGNED_IN`&&r&&!$.sesionRenderizada&&($.sesionRenderizada=!0,await t(r.user)),n===`SIGNED_OUT`&&($.sesionRenderizada=!1,e=null,$.renderLogin())});let{data:{session:n},error:r}=await a(m.auth.getSession(),8e3,ot);if(r)throw r;n?($.sesionRenderizada=!0,await t(n.user)):$.renderLogin(),window.__appBooted=!0}catch(e){window.__appBooted=!0,D.registrarOperacion(`arranque`,e),e?.message===ot&&st(),window.__showCriticalError?window.__showCriticalError(e.message||`Fallo crítico en el inicio. Verifique su conexión a internet.`):document.body.innerHTML=`<div class="p-10 text-center text-red-600 font-bold">Fallo crítico en el inicio. Verifique la consola de red.</div>`}});