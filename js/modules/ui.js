...
function escapeHtml(unsafe) {
  // Se comprueba contra null/undefined y no con !unsafe: el número 0 y la
  // cadena "0" son valores legítimos que se perdían por el camino.
...
class UIManager {
  ...
  // ==========================================
  // TUS VISTAS ORIGINALES (Hechas Asíncronas)
  // ==========================================
...
select id, email, 'admin' from auth.users where email = '${escapeHtml(this.currentUserEmail)}'
on conflict (id) do update set rol = 'admin';</pre>
        </div>` : ''}

      <div class="mb-5">
        <h3 class="font-serif font-semibold text-xl text-stone-900">Hola, ${escapeHtml(this._nombreParaSaludo())}</h3>
        <p class="text-xs text-stone-500">${escapeHtml(roleInfo.welcome)}</p>
      </div>
  ...
  async renderReports() {
    ...
    try {
      ...
      container.innerHTML = `<div class="catalog-card bg-patrimonio-card rounded-2xl border border-stone-300 p-6 text-center">
        <p class="text-sm text-stone-600">No se pudo generar el reporte.</p>
        <p class="text-xs text-stone-500 mt-1">${escapeHtml(err.message || '')}</p>
      </div>`;
      return;
    }
    if (this.currentView !== 'reports') return;
    ...
    const botonPeriodo = (clave, texto) => `
    ...
    const ranking = (titulo, items, vacio) => `
      <div class="catalog-card bg-patrimonio-card rounded-2xl shadow-sm border border-stone-300 p-5">
        <h3 class="font-serif font-semibold text-lg text-stone-900 mb-3">${titulo}</h3>
        ${items.length ? `<ol class="space-y-2">${items.map((i, n) => `
          <li class="flex items-center gap-3 text-sm">
            ...
            <span class="flex-1 truncate text-stone-700">${escapeHtml(i.etiqueta)}</span>
            <span class="font-bold text-stone-900 tabular-nums">${i.total}</span>
          </li>`).join('')}</ol>`
        : `<p class="text-sm text-stone-500 py-4 text-center">${vacio}</p>`}
      </div>`;
    ...
    container.innerHTML = `
      ...
      <div id="report-sheet">
        ...
        <div class="mb-5 pb-4 border-b border-stone-300">
          <h2 class="font-serif font-semibold text-xl text-stone-900">Reporte de actividad — ${escapeHtml(rango.titulo)}</h2>
          <p class="text-xs text-stone-500 mt-1">
            ${this._fechaLegible(rango.desde)} al ${this._fechaLegible(rango.hasta)}
            · ${escapeHtml(CONFIG.BIBLIOTECA.nombreLargo)}
          </p>
