...
on conflict (id) do update set rol = 'admin';</pre>
      ...
      <div class="mb-5">
        ...
        <p class="text-xs text-stone-500">${i.welcome}</p>
      </div>
...
`);this._descargar(`﻿`+i,`reporte-biblionexo-${t.desde}-a-${t.hasta}.csv`,`text/csv;charset=utf-8;`),this.showToast(`Reporte exportado.`,`success`)}},q=[{valor:`1`,etiqueta:`Normal`},{valor:`1.15`,etiqueta:`Grande`},{valor:`1.3`,etiqueta:`Muy grande`}];function ze(){let e=`1`;try{e=localStorage.getItem(`biblionexo-escala-fuente`)||`1`}catch{}let t=q.findIndex(t=>t.valor===e);return t===-1?0:t}function Be(e){let t=q[e];document.documentElement.style.setProperty(`--escala-fuente`,t.valor);try{localStorage.setItem(K,t.valor)}catch{}}var Ve={async renderProfile(){let e=this._container();if(!e)return;let t=null;try{t=await H.miPerfil()}catch(e){console.warn(`Perfil no disponible:`,e.message)}if(this.currentView!==`profile`)return;if(!t){e.innerHTML=this._avisoMigracion(`008`,`008_perfiles_y_permisos_librero.sql`);return}this._perfil=t;let n=l.ROLE_LABELS[t.rol]||l.ROLE_LABELS.librero,i=(t.nombre||t.email||`?`).trim().charAt(0).toUpperCase(),a=e=>e?new Date(e).toLocaleString(`es-CL`,{day:`numeric`,month:`long`,year:`numeric`,hour:`2-digit`,minute:`2-digit`}):`Nunca`,o=(e,t,n,i=``,a=``)=>r`
            ...
            <div class="min-w-0">
              ...
              ${e.genero||e.ubicacion?r`
                <div class="flex flex-wrap gap-1 mt-1">
                  ${e.genero?r`<span class="stamp stamp-info !rotate-0 !text-[9px] !py-0.5 !px-1.5"><i aria-hidden="true" class="fas fa-tag"></i> ${e.genero}</span>`:``}
                  ${e.ubicacion?r`<span class="stamp stamp-success !rotate-0 !text-[9px] !py-0.5 !px-1.5"><i aria-hidden="true" class="fas fa-location-dot"></i> ${e.ubicacion}</span>`:``}
                </div>`:``}
            </div>
          </div>
        </td>
        <td class="px-4 py-3 text-stone-500">${e.isbn}</td>
        <td class="px-4 py-3 text-center">
          <span class="${e.stock===0?`text-rose-600 font-bold`:e.stock<=1?`text-amber-600 font-semibold`:`text-emerald-700 font-semibold`}">${e.stock}</span><span class="text-stone-400 font-normal"> / ${e.copias_totales??e.stock}</span>
        </td>
        <td class="px-4 py-3 text-right whitespace-nowrap space-x-2">
          ${e.stock>0?r`<button class="loan-book-btn text-patrimonio-lago font-bold" data-id="${e.id}">Prestar</button>`:r`<button class="reserve-book-btn text-patrimonio-lago font-bold" data-id="${e.id}">Reservar</button>`}
    ...
    `)}`:r`<tr><td colspan="4" class="px-4 py-6 text-center text-stone-500">${this.userSearch?`Ningún lector coincide con la búsqueda.`:`Sin lectores registrados. Agrega el primero con el formulario de arriba.`}</td></tr>`},_bindUserRowEvents(e){e.querySelectorAll(`.delete-user-btn`).forEach(e=>{e.addEventListener(`click`,async()=>{if(await this.showConfirm(`¿Eliminar este usuario? Esta acción no se puede deshacer.`,{title:`Eliminar lector`,confirmText:`Eliminar`}))try{await Y.eliminarLector(e.dataset.id),this.showToast(`Usuario eliminado.`,`success`),this.renderUsers()}catch(e){this.showToast(e.message||`No se pudo eliminar.`,`error`)}})}),e.querySelectorAll(`.edit-user-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=(this._usersCache||[]).find(t=>String(t.id)===String(e.dataset.id));t&&this.showEditUserModal(t)})})},async renderUsers(e=!1){let t=this._container();if(!t)return;let n=this.param(`filas_por_pagina`),{lectores:i,total:a}=await Y.obtenerLectores(this.userSearch||``,this.userPage,n);if(this.currentView!==`users`)return;if(i.length===0&&this.userPage>0)return this.userPage=Math.max(0,Math.ceil(a/n)-1),this.renderUsers(e);if(this._usersCache=i,e){let e=t.querySelector(`tbody`);if(e){e.innerHTML=this._renderUserRows(i).toString();let r=t.querySelector(`#users-pagination`);r&&(r.innerHTML=this._paginacionHtml(this.userPage,a,n,`user-page-btn`)),this._bindUserRowEvents(t),this._bindPaginacion(t,`.user-page-btn`,e=>{this.userPage=e,this.renderUsers(!0)})}return}t.innerHTML=r`
      <div class="catalog-card bg-patrimonio-card rounded-2xl shadow-sm border border-stone-300 mb-6">
        <div class="catalog-card-header">
    ...
    `;let o=document.getElementById(`new-user-id`);o&&o.addEventListener(`input`,()=>{let e=o.value;if(/^[0-9kK\-\.]+$/.test(e)){let t=e.replace(/[.\-\s]/g,``).toUpperCase();t.length>1&&(o.value=`${t.slice(0,-1)}-${t.slice(-1)}`)}}),document.getElementById(`add-user-form`).addEventListener(`submit`,async e=>{if(e.preventDefault(),!this.validateUserForm(!1))return;let t=document.getElementById(`add-user-submit-btn`),n=t.innerHTML;t.disabled=!0,t.innerHTML=`<i aria-hidden="true" class="fas fa-spinner fa-spin mr-1"></i> Guardando...`;try{let e=this._datosConsentimiento(`new`);if(!e)return;let t=await Y.agregarLector({rut:this.formatRut(document.getElementById(`new-user-id`).value),nombre:document.getElementById(`new-user-name`).value.trim(),email:document.getElementById(`new-user-email`).value.trim().toLowerCase(),telefono:this.formatPhone(document.getElementById(`new-user-phone`).value),...e});this.showToast(t?.encolado?t.mensaje:`Lector agregado.`,t?.encolado?`info`:`success`),this.renderUsers()}catch(e){this.showToast(e.message||`No se pudo agregar el lector.`,`error`)}finally{t&&(t.disabled=!1,t.innerHTML=n)}}),this._bindUserRowEvents(t),this._bindConsentimiento(`new`),this._bindPaginacion(t,`.user-page-btn`,e=>{this.userPage=e,this.renderUsers(!0)});let c=document.getElementById(`user-search-input`);c.addEventListener(`input`,()=>{clearTimeout(this._userSearchTimer),this._userSearchTimer=setTimeout(()=>{this.userSearch=c.value.trim(),this.userPage=0,this.renderUsers(!0)},350)})},showEditUserModal(e){let t=document.createElement(`div`);t.className=`fixed inset-0 bg-patrimonio-lago/50 backdrop-blur-sm z-[10000] flex items-center justify-center p-4`;let n=(e,t,n,i=``)=>r`
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div class="flex flex-wrap gap-2">
          ${u(`todos`,`Todos`,c.todos,`text-stone-500`)}
      ...
      <div class="catalog-card bg-patrimonio-card rounded-2xl shadow-sm border border-stone-300 overflow-x-auto">
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            ${a.length?a.map(e=>{let t=this._estadoPrestamo(e.fecha_devolucion_esperada),n=!e.lectores?.email&&this.formatPhone(e.lectores?.telefono).length<11,i=t.clave===`vencido`?`text-rose-700 font-bold`:t.clave===`porVencer`?`text-amber-700 font-bold`:`text-stone-600`;return r`
              <tr class="border-t border-stone-200">
                <td class="px-4 py-3 font-bold text-stone-800">${e.libros?.titulo}</td>
                <td class="px-4 py-3 text-stone-600">
                  <div>${e.lectores?.nombre}</div>
                  <div class="text-[11px] text-stone-500 font-mono">${e.lectores?.rut||``}</div>
                </td>
                <td class="px-4 py-3 ${i}">
                  <div>${this._fechaLegible(e.fecha_devolucion_esperada)}</div>
                  ${t.clave===`vencido`?r`<span class="stamp stamp-danger mt-1"><i aria-hidden="true" class="fas fa-triangle-exclamation"></i> ${t.etiqueta}</span>`:r`<div class="text-[11px] font-medium ${t.clave===`porVencer`?`text-amber-700`:`text-stone-500`}">${t.etiqueta}</div>`}
                </td>
                <td class="px-4 py-3 text-right whitespace-nowrap space-x-3">
                  ...
                  ${t.clave!==`vencido`&&(e.renovaciones??0)<this.param(`max_renovaciones`)?r`
            ...
            `}):r`<tr><td colspan="4" class="px-4 py-8 text-center text-stone-500">${t===`vencidos`?`No hay préstamos atrasados.`:t===`porVencer`?`No hay préstamos por vencer.`:`No hay préstamos activos.`}</td></tr>`}
          </tbody>
        </table>
        ...
        <div class="overflow-y-auto px-6 divide-y divide-stone-200 border-t border-stone-200">
          ${e.map(e=>{let t=this._estadoPrestamo(e.fecha_devolucion_esperada);return r`
            <div class="py-3 flex items-center justify-between gap-3">
              <div class="min-w-0">
                <p class="font-bold text-stone-800 text-sm truncate">${e.lectores?.nombre}</p>
                <p class="text-xs text-stone-500 truncate">${e.libros?.titulo}</p>
    ...
    `.toString(),document.body.appendChild(t);let n=this._prepararModal(t);t.querySelector(`[data-action="close"]`).addEventListener(`click`,n),t.addEventListener(`click`,e=>{e.target===t&&n()}),t.querySelectorAll(`[data-notify-id]`).forEach(t=>{t.addEventListener(`click`,()=>{let n=e.find(e=>String(e.id)===String(t.dataset.notifyId));n&&this.showNotifyModal(n)})})},async _seleccionarLectorModal(e){return new Promise(t=>{let n=document.createElement(`div`);n.className=`fixed inset-0 bg-patrimonio-lago/50 backdrop-blur-sm z-[10000] flex items-center justify-center p-4`,n.innerHTML=`
        <div class="bg-patrimonio-card border border-stone-300 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
          <h3 class="font-serif text-lg font-bold text-stone-900">${escapeHtml(e)}</h3>
      ...
      `,document.body.appendChild(n);let r=n.querySelector(`#lector-search-input`),i=n.querySelector(`#lector-search-results`),a=n.querySelector(`#lector-search-confirm`),o,s=null,c=()=>{n.remove(),t(null)},l=(e,t)=>{s=e,r.value=e,i.innerHTML=`
          ...
          </div>`,a.disabled=!1,a.focus()};n.querySelector(`#lector-search-cancel`).addEventListener(`click`,c),n.addEventListener(`click`,e=>{e.target===n&&c()}),a.addEventListener(`click`,()=>{let e=s||r.value.trim();n.remove(),t(e)}),r.addEventListener(`input`,()=>{let e=r.value;if(/^[0-9kK\-\.]+$/.test(e)){let t=e.replace(/[.\-\s]/g,``).toUpperCase();t.length>1&&(r.value=`${t.slice(0,-1)}-${t.slice(-1)}`)}a.disabled=!r.value.trim(),s=null,clearTimeout(o);let t=r.value.trim();if(t.length<2){i.innerHTML=``;return}o=setTimeout(async()=>{i.innerHTML=`<p class="text-xs text-stone-400 p-2"><i aria-hidden="true" class="fas fa-spinner fa-spin mr-1"></i> Buscando...</p>`;try{let e=await Y.obtenerLectores(t,0,5);e.lectores.length===0?i.innerHTML=`<p class="text-xs text-stone-400 p-2">Ningún lector coincide. Escriba el RUT completo para registrarlo como nuevo.</p>`:(i.innerHTML=e.lectores.map(e=>`
                <button type="button" data-rut="${e.rut}" data-nombre="${escapeHtml(e.nombre)}" class="w-full text-left px-3 py-2 rounded-lg border border-transparent hover:bg-stone-50 hover:border-stone-200 focus:bg-stone-50 focus:border-stone-200 focus:outline-none transition-colors">
                  <p class="text-sm font-medium text-stone-800">${escapeHtml(e.nombre)}</p>
                  <p class="text-[11px] font-mono text-stone-500">${e.rut}</p>
                </button>
        ...
        <button data-action="ver-prestamos" class="btn-secundario border border-stone-300 bg-white text-stone-700 px-4 py-2 rounded-xl text-sm font-medium">Ver sus préstamos</button>`):(o=r`
        <div class="bg-patrimonio-lago/5 border border-patrimonio-lago/20 rounded-xl p-4 text-center">
          ...
          <p class="text-sm text-stone-600 mt-1">El RUT <span class="font-mono font-bold">${t}</span> no está registrado.</p>
