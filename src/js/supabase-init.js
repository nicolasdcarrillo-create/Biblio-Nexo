...
let client = null;
if (!window.supabase) {
    // BUG-06: Separamos explícitamente los dos casos de error para no confundir
    // un fallo de createClient (CORS, config inválida) con un CDN no disponible.
    // Este primer caso es "el script del CDN nunca llegó".
    ...
    try {
        ...
        // El CDN sí cargó, pero createClient falló (config inválida, CORS, etc.)
        console.error('Error crítico: Supabase CDN disponible pero createClient falló:', err.message);
    }
}
